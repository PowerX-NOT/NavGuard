...
object TextUtil {
    ...
    const val caretBackground = 0xff666666.toInt()
    ...
    fun fromHexString(s: CharSequence): ByteArray {
        ...
        for (pos in s.indices) {
            ...
            val c = s[pos].code
            when {
                c in '0'.code..'9'.code -> { nibble++; b = (b * 16 + (c - '0'.code)).toByte() }
                c in 'A'.code..'F'.code -> { nibble++; b = (b * 16 + (c - 'A'.code + 10)).toByte() }
                c in 'a'.code..'f'.code -> { nibble++; b = (b * 16 + (c - 'a'.code + 10)).toByte() }
            }
        }
